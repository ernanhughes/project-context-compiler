 ...
 */
function illegality(c) {
    if (c.scopeEligible !== true)
        return "scope";
    if (c.freshnessEligible !== true)
        return "freshness";
    if (c.authorityEligible !== true)
        return "authority";
    if (c.formRank < c.minRank)
        return "floor";
    return null;
}
export function validateBundle(bundle, request, candidates, _policy) {
    void _policy;
    const problems = [];
    const byId = new Map(candidates.map((c) => [c.candidateId, c]));
    const liveIds = bundle.items.map((item) => item.id);
    if (liveIds.length !== bundle.layoutTrace.length ||
    ...
    if (total > request.usableTokenBudget) {
        problems.push(`rendered cost ${total} exceeds budget ${request.usableTokenBudget}`);
    }
    ...
    for (const itemId of liveIds) {
        ...
        if (seenContent.has(candidate.contentIdentity)) {
            problems.push(`duplicate representation admitted: ${candidate.contentIdentity}`);
        }
        seenContent.add(candidate.contentIdentity);
    }
    return problems;
}
export function validateResult(result, request, candidates) {
    const problems = [];
    const traced = new Set(result.trace.entries.map((e) => e.candidateId));
    const considered = new Set(candidates.map((c) => c.candidateId));
    if (result.success) {
        if (traced.size !== considered.size ||
            [...considered].some((id) => !traced.has(id))) {
            problems.push(`trace covers ${traced.size} of ${considered.size} considered candidates`);
        }
    }
...
function transitiveDependencies(start, byId) {
 ...
 */
function traceSemantics(result, candidates) {
    const problems = [];
    const byId = new Map(candidates.map((c) => [c.candidateId, c]));
    for (const entry of result.trace.entries) {
        ...
        if (entry.dependencyClosure.length !== expectedClosure.length ||

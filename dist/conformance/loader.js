...
import { candidateFromJSON, requestFromJSON, } from "../core/domain.js";
export const CANDIDATE_KEYS = new Set([
    "candidate_id",
    "content_identity",
    "representation_id",
    "form_rank",
...
function rejectUnknown(record, allowed, path, what) {
...
export function loadCandidateFile(path) {
    ...
    const loaded = [];
    for (const record of raw["candidates"]) {
...
export function loadFixtureSet(root) {
    const found = {};
    for (const entry of readdirSync(root).sort()) {
        if (!entry.endsWith(".candidates.json"))
            continue;

 ...
 */
import { bundleToJSON, compileContext, defaultPolicy, renderBundleText, resultToJSON, validateBundle, validateResult, } from "../core/index.js";
import { bundleFromJSON, candidateFromJSON, policyFromJSON, requestFromJSON, resultFromJSON, traceFromJSON, } from "../core/index.js";
...
function strInput(args, key, required) {
...
function candidateList(root, args) {
    const inline = args["candidates"];
    if (typeof inline === "string") {
        const parsed = JSON.parse(inline);
        const list = parsed["candidates"] ?? parsed;
        if (!Array.isArray(list))
            throw new Error("inline candidates must be a list");
        return list.map(candidateFromJSON);
    }
    const file = strInput(args, "candidates_file", true);
    const parsed = readJSONFile(root, file);
    const list = parsed["candidates"];
    if (!Array.isArray(list))
        throw new Error("candidates file must hold {candidates: [...]}");
    return list.map(candidateFromJSON);
}
function requestDoc(root, args) {
    const inline = args["request"];
    if (typeof inline === "string") {
        return requestFromJSON(JSON.parse(inline));
    }
    const file = strInput(args, "request_file", true);
    return requestFromJSON(readJSONFile(root, file));
}
function policyDoc(root, args) {
    const inline = args["policy"];
    if (typeof inline === "string") {
        return policyFromJSON(JSON.parse(inline));
    }
...
export function compileTool(workdir) {
    return {
        name: "context_compiler_compile",
        description: "Deterministically compile explicit context candidates under a policy and budget. " +
        ...
        input: {
            ...
            properties: {
                ...
                candidates: {
                    type: "string",
                    description: "Inline candidates JSON (alternative to candidates_file)",
                },
                policy: {
                    type: "string",
        ...
        async execute(input) {
            ...
            return textResult({
                ...
                rendered_text: output.bundle && includeRendered
...
export function validateTool(workdir) {
    return {
        name: "context_compiler_validate",
        description: "Validate an existing bundle/result envelope against request/candidates/policy. " +
        ...
        input: {
            ...
            properties: {
                ...
                compilation: {
                    type: "string",
                    description: "Inline {bundle, result} JSON (alternative to compilation_file)",
                },
            },
            additionalProperties: false,
        },
        async execute(input) {
            ...
            const doc = typeof inline === "string"
                ? JSON.parse(inline)
                : readJSONFile(root, strInput(args, "compilation_file", true));
...
export function inspectTool() {
    return {
        ...
        description: "Inspect a DecisionTrace (inline JSON or worktree file), optionally filtered to one " +
        ...
        async execute(input) {
            ...
            const root = typeof args["workdir"] === "string" ? args["workdir"] : process.cwd();
            const doc = typeof inline === "string"
                ? JSON.parse(inline)
                : readJSONFile(root, strInput(args, "trace_file", true));
            const trace = traceFromJSON(doc);
            const only = args["candidate_id"];
            const entries = typeof only === "string"

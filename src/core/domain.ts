...
export const COMPILATION_RESULT_SCHEMA =
  "project_context.compilation_result.v2";
export const COMPILATION_RESULT_SCHEMA_V1 =
...
export function traceEntryToV1JSON(e: TraceEntry): Record<string, unknown> {
...
export function traceToV1JSON(t: DecisionTrace): Record<string, unknown> {
  return {
...
export function traceToJSON(t: DecisionTrace): Record<string, unknown> {
  if (t.legacySchema === "v1") return traceToV1JSON(t);
  return {
...
export function traceFromJSON(data: unknown): DecisionTrace {
  ...
  return version === DECISION_TRACE_SCHEMA_V1
    ? { ...trace, legacySchema: "v1" as const }
    : trace;
}
...
export function resultToV1JSON(r: CompilationResult): Record<string, unknown> {
  return {
    ...resultToJSON(r),
...
export function resultFromJSON(data: unknown): CompilationResult {
  ...
  if (
    version !== COMPILATION_RESULT_SCHEMA &&
